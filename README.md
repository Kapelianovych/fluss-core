...
Delays function invocation based on number of frames (one frame is about _16.67ms_). It ensures that function will be executed after this time. By default, it delays function with two frames. If _frames_ is set to `0` or `1`, then, if present, `requestAnimationFrame` is used. Otherwise, `setTimeout` function is in use.
